...
func (r *reviewRepo) Create(ctx context.Context, rev *models.Review) error {
	query := `
		INSERT INTO reviews (reviewer_id, provider_id, rating, text)
		VALUES ($1, $2, $3, $4)
		RETURNING id, status, created_at, updated_at`
	err := r.db.QueryRowContext(ctx, query,
		rev.ReviewerID, rev.ProviderID, rev.Rating, rev.Text,
...
func (r *reviewRepo) GetByID(ctx context.Context, id string) (*models.Review, error) {
	...
	err := r.db.GetContext(ctx, &rev,
		`SELECT id, reviewer_id, provider_id, rating, text, status, provider_response, created_at, updated_at
